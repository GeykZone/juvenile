...
if(isset($_GET['offenses']))
{
    ...
    if($query_click == 'unclicked')
    {
        ...
        WHERE (date_of_offense BETWEEN '$date_range_from' AND '$date_range_to') AND `barangay_tbl_id`='$crime_location' ORDER BY `offense_name` ASC";
    }
    else
    {
    ...
    if ($result->num_rows > 0)
    {
        while($row = $result->fetch_assoc())
        {
            ...
            if($query_click == 'unclicked')
            {
                ...
                WHERE (date_of_offense BETWEEN '$date_range_from' AND '$date_range_to') AND `barangay_tbl_id`='$crime_location' AND offense_name = '$offense_name' ORDER BY `offense_name` ASC";
            }
            else
            {
...
if(isset($_GET['time']))
{
    ...
    {
        $sql = "SELECT * FROM `jd_tbl` AS `j` LEFT JOIN `barangay_tbl` AS `b` ON (`j`.`barangay_tbl_id` = `b`.`id`) LEFT JOIN `offense_tbl` AS `o` ON (`j`.`offense_id` = `o`.`id`)
        WHERE (date_of_offense BETWEEN '$date_range_from' AND '$date_range_to') AND `barangay_tbl_id`='$crime_location' ORDER BY `date_of_offense` ASC";
    }
    else
    {
        $query = "SELECT * FROM `jd_tbl` AS `j` LEFT JOIN `barangay_tbl` AS `b` ON (`j`.`barangay_tbl_id` = `b`.`id`) LEFT JOIN `offense_tbl` AS `o` ON (`j`.`offense_id` = `o`.`id`)";
    ...
    if ($result->num_rows > 0)
    {
        while($row = $result->fetch_assoc())
        {
            ...
            {
                $new_sql = "SELECT * FROM `jd_tbl` AS `j` LEFT JOIN `barangay_tbl` AS `b` ON (`j`.`barangay_tbl_id` = `b`.`id`) LEFT JOIN `offense_tbl` AS `o` ON (`j`.`offense_id` = `o`.`id`)
                WHERE date_of_offense = '$date_of_offense' AND `barangay_tbl_id`='$crime_location' ORDER BY `date_of_offense` ASC";
            }
            else
            {
